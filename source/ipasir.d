...
class Solver {
    ...
    private void add0(Literal l) {
        if(l > maximum) maximum = l;
        ipasir_add(this.internalSolver, l);
    }

    void add(in Clause clause){
        if(clause.length>0) {
            foreach(lit; clause) add0(lit);
            ipasir_add(this.internalSolver, 0);
        }
    }

    void add(A...)(A literals)
    {
        foreach(t; literals) add0(t);
        ipasir_add(this.internalSolver, 0);
    }

    void assume(in Literal lit) {
        ipasir_assume(this.internalSolver, lit);
    }


    void assume(in Clause clause) {
        foreach(t; clause)
            ipasir_assume(this.internalSolver, t);
    }
